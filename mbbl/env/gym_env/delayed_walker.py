...
class env(base_env_wrapper.base_env):
    ...
    WALKER = ['gym_dfwalker2d', 'gym_dfhopper', 'gym_dfant']
    ...
    def __init__(self, env_name, rand_seed, misc_info):
        ...
        if 'reset_type' in misc_info and misc_info['reset_type'] == 'gym':
            ...
            self.observation_space = gym.spaces.Box(-np.ones(self._env_info['ob_size']), \
                    np.ones(self._env_info['ob_size']))
    ...
    def step(self, action):
        ...
        if done or self._done_counter >= 0:
            self._done_counter += 1

        if self._done_counter > 100 or self._current_step >= self._env_info['max_length']:
            done = True
        else:
            done = False
    ...
    def _get_observation(self):
        ...
        if self._env_name in ['gym_dfwalker2d', 'gym_dfhopper']:
            return np.concatenate([qpos[1:], qvel]).ravel()
        elif self._env_name in ['gym_dfant']:
    ...
    def _set_dynamics_api(self):
        ...
        def set_state(data_dict):
            ...
            old_state = self._env.env.sim.get_state()
            new_state = mujoco_py.MjSimState(old_state.time, qpos, qvel,
                    old_state.act, old_state.udd_state)
            self._env.env.sim.set_state(new_state)
    ...
    def _set_reward_api(self):
        ...
        velocity_ob_pos = {
            'gym_cheetah': 8, 'gym_dfwalker2d': 8, 'gym_dfhopper': 5,
            'gym_dfswimmer': 3, 'gym_dfant': 13
        }[self._env_name]
        height_ob_pos = {
            'gym_cheetah': -1, 'gym_dfwalker2d': 0, 'gym_dfhopper': 0,
            'gym_dfswimmer': -1, 'gym_dfant': 0
        }[self._env_name]

        target_height = {
            'gym_cheetah': -1, 'gym_dfwalker2d': 1.3, 'gym_dfhopper': 1.3,
            'gym_dfswimmer': -1, 'gym_dfant': 0.57
        }[self._env_name]

        height_coeff = {
            'gym_cheetah': 0.0, 'gym_dfwalker2d': 3, 'gym_dfhopper': 3,
            'gym_dfswimmer': 0.0, 'gym_dfant': 3
        }[self._env_name]
        # MBMF paper coefficient. For hopper and ant, there is length award.
        ctrl_coeff = {
            'gym_cheetah': 0.1, 'gym_dfwalker2d': 0.1, 'gym_dfhopper': 0.1,
            'gym_dfswimmer': 0.0001, 'gym_dfant': 0.1
        }[self._env_name]
        ...
        def reward(data_dict):
            ...
            elif self._env_name == 'gym_dfswimmer':
                alive_reward = 0.0
