...
from ebm_rl.brac import train_eval_utils
...
def train_eval_ebm(
  ...
  interval = 1
  for _train_step in range(total_train_steps):
    collector.collect_transition()
    agent._global_step.assign_add(1)
    if _train_step % interval == 0:
      for _ in range(1):
        # model_idx = random.randint(0, len(agent._qembed_fns)-1)
        # agent._model_idx.assign(model_idx)
        # agent._test_policies['main']._idx.assign(model_idx)
        # agent._online_policies['main']._idx.assign(model_idx)
        agent.train_step()
      '''
      for _ in range(interval):
        train_batch = agent._get_train_batch()
        model_info = agent._optimize_model_alone(train_batch)
      f_net_source_vars, f_net_target_vars = agent._get_source_target_f_net_vars()
      qembed_source_vars, qembed_target_vars = agent._get_source_target_embed_vars()
      utils.soft_variables_update(f_net_source_vars, qembed_source_vars, tau=1.0)
      for _ in range(interval):
        train_batch = agent._get_train_batch()
        info = agent._optimize_step(train_batch)
      info.update(model_info)
      for key, val in info.items():
        agent._train_info[key] = val.numpy()
      '''
    ...
    if step % eval_freq == 0 or step == total_train_steps:
      ...
      # for model_idx in range(len(agent._qembed_fns)):
      #   agent._model_idx.assign(model_idx)
      #   agent._test_policies['main']._idx.assign(model_idx)
      #   agent._online_policies['main']._idx.assign(model_idx)
