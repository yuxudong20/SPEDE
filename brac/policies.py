...
class DecoupleDeterministicSoftPolicyNoisy(tf.Module):
  """Returns mode of policy distribution."""

  def __init__(self, a_embed, a_network):
    super(DecoupleDeterministicSoftPolicyNoisy, self).__init__()
    self._a_network = a_network
    self._a_embed = a_embed

  @tf.function
  def __call__(self, observation, state=()):
    action = self._a_network(self._a_embed(observation), remove_noise=True)[0]
    return action, state

class DecoupleDeterministicSoftPolicy(tf.Module):
  """Returns mode of policy distribution."""

  def __init__(self, a_embed, a_network):
    super(DecoupleDeterministicSoftPolicy, self).__init__()
    self._a_network = a_network
    self._a_embed = a_embed

  @tf.function
  def __call__(self, observation, state=()):
    action = self._a_network(self._a_embed(observation))[0]
    return action, state

class DecoupleRandomSoftPolicy(tf.Module):
  """Returns sample from policy distribution."""

  def __init__(self, a_embed, a_network):
    super(DecoupleRandomSoftPolicy, self).__init__()
    self._a_network = a_network
    self._a_embed = a_embed

  @tf.function
  def __call__(self, observation, state=()):
    action = self._a_network(self._a_embed(observation))[1]
    return action, state

class DecoupleDeterministicSoftPolicy2(tf.Module):
  """Returns mode of policy distribution."""

  def __init__(self, a_embed, a_network):
    super(DecoupleDeterministicSoftPolicy2, self).__init__()
    self._a_network = a_network
    self._a_embed = a_embed
    self._idx = tf.Variable(0, trainable=False)

  @tf.function
  def __call__(self, observation, state=()):
    actions = []
    for a_network, a_embed in zip(self._a_network, self._a_embed):
      actions.append(a_network(a_embed(observation))[0])
    actions = tf.stack(actions, axis=0)
    action = tf.gather(actions, self._idx, axis=0)
    return action, state


class DecoupleRandomSoftPolicy2(tf.Module):
  """Returns sample from policy distribution."""

  def __init__(self, a_embed, a_network):
    super(DecoupleRandomSoftPolicy2, self).__init__()
    self._a_network = a_network
    self._a_embed = a_embed
    self._idx = tf.Variable(0, trainable=False)
  ...
  def __call__(self, observation, state=()):
    actions = []
    for a_network, a_embed in zip(self._a_network, self._a_embed):
      actions.append(a_network(a_embed(observation))[1])
    actions = tf.stack(actions, axis=0)
    action = tf.gather(actions, self._idx, axis=0)
    return action, state
