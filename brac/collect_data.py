...
from ebm_rl.brac import policy_loader
...
flags. DEFINE_float('eps_portion', 0.1, 'portional of random actions')
...
def collect_n_transitions(tf_env, policy, data, n, eps_portion, log_freq=10000):
  """Adds desired number of transitions to dataset."""
  collector = train_eval_utils.DataCollector(tf_env, policy, data, eps_portion)
  time_st = time.time()
  timed_at_step = 0
  steps_collected = 0
...
def collect_data(
  ...
  for (policy_name, policy_cfg, _), n_transitions in zip(
    ...
    collect_n_transitions(tf_env, policy, data, n_transitions, eps_portion, log_freq)
